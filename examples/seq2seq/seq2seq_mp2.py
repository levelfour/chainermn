...
def get_subsequence(xs, i, n):

    if i < n - 1:
        def _get_subsequence(x):
            if len(x) == 1:
                # Prevent the length of sequence becomes 0.
                return x
            else:
        ...
        return [x[i*len(x)//n:] for x in xs]
...
class Encoder(chainer.Chain):
    ...
    def __call__(self, *inputs):
        ...
        xs = get_subsequence(xs, seq_index, split_size)
...
class Decoder(chainer.Chain):
    ...
    def __call__(self, *inputs):
        ...
        # Split the output sequence and fetch the subsequence.
        split_size = self.comm.size // 2
        seq_index = self.comm.rank % split_size
        ys_in = get_subsequence(ys_in, seq_index, split_size)
        ys_out = get_subsequence(ys_out, seq_index, split_size)
    ...
    def translate(self, xs, max_length=100):
        # Decoder test phase can be only done on one process
        # because each decoder subprocess shares the same parameters.
        if self.comm.rank != self.comm.size // 2:
            return None
        ...
        with chainer.no_backprop_mode():
            with chainer.using_config('train', False):
                ...
                h, c = self.mn_decoder.recv_states()
...
class BleuEvaluator(extensions.Evaluator):
    def __init__(self, model, test_data, device=-1, batch=100,
        ...
        # In evaluation, we use only one process for decoder.
        self.is_encoder = self.comm.rank < self.comm.size // 2
        self.is_decoder = self.comm.rank == self.comm.size // 2
...
def main():
    ...
    if args.gpu:
        world_comm = chainermn.create_communicator('hierarchical')
        # First half of processes will be assigned to the encoder,
        # the other to the decoder.
        is_encoder = world_comm.rank < world_comm.size // 2
        local_comm = world_comm.split(is_encoder, world_comm.rank)
        dev = world_comm.intra_rank
    else:
        world_comm = chainermn.create_communicator('naive')
        # First half of processes will be assigned to the encoder,
        # the other to the decoder.
        is_encoder = world_comm.rank < world_comm.size // 2
        local_comm = world_comm.split(is_encoder, world_comm.rank)
    ...
    if world_comm.size != 4:
        raise ValueError(
            'This example can only support exactly 4 processes.')
    ...
    # Every processes prepare datasets.
    if True:
    ...
    # Print GPU id
    for i in range(0, world_comm.size):
        if world_comm.rank == i:
            print("Rank {} GPU: {}".format(world_comm.rank, dev))
        sys.stdout.flush()
        world_comm.mpi_comm.Barrier()

    # broadcast id -> word dictionary
    source_ids = world_comm.mpi_comm.bcast(source_ids, root=0)
    target_ids = world_comm.mpi_comm.bcast(target_ids, root=0)
    ...
    # At the end of each step of optimization, allreduce grad
    # among encoder / decoder.
    optimizer = create_optimizer(args.optimizer)
    optimizer = chainermn.create_multi_node_optimizer(optimizer, local_comm)
